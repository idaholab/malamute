...
plt.plot(abs(df.loc[:,strain_name])*1e3,\
         df.loc[:,stress_name]/1e6,\
        '-', color='red', alpha=1.0, linewidth=3, label='300')
df=pd.read_csv('t400.csv')
plt.plot(abs(df.loc[:,strain_name])*1e3,\
         df.loc[:,stress_name]/1e6,\
        '-', color='blue', alpha=1.0, linewidth=3, label='400')
df=pd.read_csv('t500.csv')
plt.plot(abs(df.loc[:,strain_name])*1e3,\
         df.loc[:,stress_name]/1e6,\
        '-', color='green', alpha=1.0, linewidth=3,label='500')
df=pd.read_csv('t600.csv')
plt.plot(abs(df.loc[:,strain_name])*1e3,\
         df.loc[:,stress_name]/1e6,\
        '-', color='cyan', alpha=1.0, linewidth=3, label='600')
