...
malamute = pd.read_csv(directory + "fourspacer_twopunch_diewall_threephysics_out.csv")
...
plt.figure()
ax1 = malamute.plot.line(
    x="time", y="pyrometer_point", color="slategrey", label="Pyrometer point"
)
dcs5.plot.line(
    ax=ax1,
    x="time",
...
ax2 = malamute.plot.line(
    x="time",
    y="upper_plunger_step_point",
    color="peru",
    marker="o",
    markevery=3,
    label="Upper plunger",
...
ax3 = malamute.plot.line(
    x="time",
    y="upper_plunger_center_point",
    color="orange",
    marker="^",
    markevery=5,
    label="Upper plunger",
