...
[Problem]
  ...
  group_variables = 'disp_x disp_y; electric_potential potential_stainless_steel; temperature temperature_stainless_steel'
[]
...
[BCs]
  ...
  [electric_potential_top]
    ...
    function = 'if(t<10, 0 , if(t < 41, (980 / (pi * 0.00155))*((0.141301/4.3625)*(t-10)), if(t > 610, 0, 980 / (pi * 0.00155))))' # RMS Current / Cross-sectional Area. Ramping for 10 < t < 41s approximately reflects Cincotti et al (DOI: 10.1002/aic.11102) Figure 21(b)
  []
  ...
  [centerline_no_dispx]
    ...
    variable = disp_x
    value = 0
    boundary = 'centerline_powder_compact centerline_upper_plunger centerline_lower_plunger centerline_spacers centerline_steel_ram'
  []
  [fixed_in_y]
    type = ADDirichletBC
    preset = true
    variable = disp_y
    value = 0
    boundary = 'axial_centerpoint centerpoint_outer_die_wall' # bottom_lower_steel_ram top_upper_steel_ram'
  []
  [bottom_pressure_ydirection]
    type = ADPressure
    variable = disp_y
    boundary = 'bottom_lower_steel_ram'
    function = 'if(t<10.0, (1.33e6/10.0)*t, 1.33e6)'
    component = 1
  []
...
[Contact]
  [upper_plunger_powder_mechanical]
    ...
    model = frictionless
    penalty = 1e10 #1e12 worked for isotropic yttria #1e14 worked when trying creep #1e4 is good in the mm-MPa system
...
[Materials]
  ...
  [stainless_steel_electrical_conductivity]
    ...
    block = stainless_steel
  []
  [stainless_steel_elasticity_tensor]
    type = ADComputeIsotropicElasticityTensor
    youngs_modulus = 1.93e10 #in Pa, 19.3 GPa, Cincotti
    poissons_ratio = 0.29
    block = stainless_steel
  []
  [stainless_steel_stress]
    type = ADComputeFiniteStrainElasticStress
    block = stainless_steel
  []
...
##  harmonic mean of graphite and stainless steel hardness
  ...
  [yttria_elasticity_tensor]
    ...
    youngs_modulus = 1.653e10 #in Pa assuming 64% initial density and theoretical coeff. from Phani and Niyogi (1987)
  ...
  [yttria_thermal_expansion]
    ...
    thermal_expansion_coeff = 9.3e-6 #Curtis, Properties of Yttrium Oxide Ceramics, Journal of the American Ceramic Society (1957), 40(8) pg 274
    stress_free_temperature = ${initial_temperature}
    temperature = temperature
    block = powder_compact
  []
...
[Executioner]
  ...
  nl_abs_tol = 8e-12 #was 2e-12, before that 1e-12
  nl_max_its = 20
  l_max_its = 50
  dtmin = 1.0e-4
