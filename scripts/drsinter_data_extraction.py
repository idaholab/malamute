...
if (
    smooth_disp_query == "Y"
    or smooth_disp_query == "y"
    or smooth_disp_query == "Yes"
    or smooth_disp_query == 1
    or smooth_disp_query == "True"
):
    smooth_disp = True
    ...
    smooth_disp_query == "N"
    or smooth_disp_query == "n"
    or smooth_disp_query == "No"
    or smooth_disp_query == 0
    or smooth_disp_query == "False"
):
    smooth_disp = False
else:
...
if (
    smooth_temperature_query == "Y"
    or smooth_temperature_query == "y"
    or smooth_temperature_query == "Yes"
    or smooth_temperature_query == 1
    or smooth_temperature_query == "True"
):
    smooth_temperature = True
elif smooth_temperature_query == "":
    smooth_temperature = True
elif (
    smooth_temperature_query == "N"
    or smooth_temperature_query == "n"
    or smooth_temperature_query == "No"
    or smooth_temperature_query == 0
    or smooth_temperature_query == "False"
):
...
for i in range(1, len(smoothing_row.columns)):
    ...
    if isNaN(curr_state) == False:
        ...
        metadata.loc[i + len(units_row.columns) + j] = [
            "Smoothing for " + curr_channel,
            curr_state,
        ]
        if curr_state == "ON":
            j += 1
...
end_header_row_index = (
    raw_data[0].loc[raw_data[0][dummy_names[0]] == "#EndHeader"].index[0]
)
...
metadata.loc[len(units_row.columns) + len(smoothing_row.columns) + j + 1] = [
...
if smooth_disp:
    ...
    metadata.loc[len(units_row.columns) + len(smoothing_row.columns) + j + 2] = [
        "Displacement smoothing (postprocess)",
        "ON",
    ]
    metadata.loc[len(units_row.columns) + len(smoothing_row.columns) + j + 3] = [
        "Displacement smoothing window size",
        disp_smoothing_window,
    ]
...
if smooth_temperature:
    ...
    metadata.loc[len(units_row.columns) + len(smoothing_row.columns) + j + 4] = [
        "Temperature smoothing (postprocess)",
        "ON",
    ]
    metadata.loc[len(units_row.columns) + len(smoothing_row.columns) + j + 5] = [
        "Temperature smoothing window size",
...
reindex_dict = {
    exp_data.index[i]: new_row_indices[i] for i in range(len(new_row_indices))
}
...
if smooth_disp:
    exp_data["Z-AXIS"] = (
        exp_data["Z-AXIS"]
        .rolling(window=disp_smoothing_window, min_periods=1, center=True)
        .mean()
    )
    print(
        "Experimental displacement data smoothed with",
        disp_smoothing_window,
        "data points per averaging window...",
    )
...
if smooth_temperature:
    exp_data["TEMP."] = (
        exp_data["TEMP."]
        .rolling(window=temp_smoothing_window, min_periods=1, center=True)
        .mean()
    )
    print(
        "Experimental temperature data smoothed with",
        temp_smoothing_window,
        "data points per averaging window...",
    )
#########################################################
...
if plot_data:
    ...
    for i in range(1, len(cols)):
        exp_data.plot(
            ...
            x="TIME",
            y=cols[i],
            color=color_and_marker[0][0],
            marker=color_and_marker[0][1],
            markerfacecolor=color_and_marker[0][2],
            markevery=int(color_and_marker[0][3]),
            xlabel="Time (s)",
            ylabel=(cols[i] + " (" + units[i] + ")"),
        )
        ax1.relim()
        ax1.autoscale_view()
